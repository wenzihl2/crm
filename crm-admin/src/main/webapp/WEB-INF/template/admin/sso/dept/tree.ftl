    ...
    $(function() {
    	$("#navtab").ligerTab({
    		onBeforeSelectTabItem:function(tabid){
    			if(tabid == "show_all") {
    				location.href = "${base}/admin/sso/dept/tree.jhtml?async=" + async;
    			} else if(tabid == "show_eq") {
    				location.href = "${base}/admin/sso/dept/tree.jhtml?search.show_eq=true&async=" + async;
    			}
            }
        });
        var zNodes =[
            ...
            setting : {
                callback : {
                    onClick: function(event, treeId, treeNode, clickFlag) {
                        parent.frames['listFrame'].location.href='${base}/admin/sso/dept/' + treeNode.id + "/editForm.jhtml?async=" + async;
                    }
                }
            }
        });
