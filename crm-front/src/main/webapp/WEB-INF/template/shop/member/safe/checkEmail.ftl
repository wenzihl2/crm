...
<script type="text/javascript">
	function dosubmit() {
		...
		if (checkEmail(email) && checkRecoverKey()) {
			$("#inputForm").submit();
		}
	}
	...
	function checkEmail(email) {
		if ($.trim(email) == "") {
			...
			$("#email_message").css("display", "");
			return false;
		}
		var reg = /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((\.[a-zA-Z0-9_-]{2,3}){1,2})$/;
		if (!reg.test(email)) {
			$("#email_message").html("请正确填写邮箱格式！");
			$("#email_message").css("display", "");
			return false;
		}
	...
	function sendMessage() {
		$.ajax({
			url : base + "/member/safe/emailVerify/sendSms",
			type : 'post',
...
<body class="login" mycenter="member_safe">
	...
	<div id="content">
		...
		<div class="right">
			<div class="mydd">
				<div class="mydd_m">绑定邮箱</div>
				<div class="mydd_c">
					<div id="step2" class="step">
						<ul>
							<li class="fore1">1.验证邮箱<b></b></li>
							<li class="fore2">2.完成</li>
						</ul>
					</div>
					<form id="inputForm" class="validate" action="updateEmail" method="post">
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0"  class="liuyan">
							<tbody>
								...
								<td align="right">请填写手机验证码：</td>
									<td>
										<input name="smsRecoverKey" id="member_recoverKey" type="text" size="30" class="inputBg" maxlength="13">
								...
								<tr>
									...
									<td>
										<input type="text" name="email" id="member_email" size="30" class="inputBg" value="${(member.email)!''}" maxlength="64">
										<span id="email_message" class="ts_on" style="display:none; color:red;">请正确填写邮箱格式</span>
									</td>
								</tr>
								<tr>
									<td align="right" valign="top">&nbsp;</td>
									<td height="45">
										<input class="btn-input-red63" type="button" onclick="javascript:dosubmit()" value="提 交"/>
										&nbsp;&nbsp;
