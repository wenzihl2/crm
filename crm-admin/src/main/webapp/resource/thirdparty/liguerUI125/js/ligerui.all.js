...
(function ($)
{
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function () {
            ...
            // 如果是autoComplete则使用....作为下拉框
            if(p.autocomplete == true) {
            	g.link.find("div:first").removeClass('l-trigger-icon');
            	p.triggerIcon = "l-trigger-icon-autocomplete";
            }
        ...
        _setUrl: function (url) {
            ...
            if (p.autocomplete && !p.autocompleteAllowEmpty && !currentKey){
            	return;
            }
            $.ajax({
                type: p.ajaxType,
        ...
        setTree: function (tree) {
            ...
            tree.onAfterAppend = function (domnode, nodedata) {
                ...
                g.selectValueByTree(value, isTrigger);
            };
            g.tree = $("<ul></ul>");
        ...
        selectValueByTree: function (value, isTrigger) {
            var g = this, p = this.options;
            if (value != null) {
                var text = "";
                ...
                g._changeValue(value, text, isTrigger);
            }
        },
        //表格
        ...
        //设置值到 文本框和隐藏域
        _changeValue: function (newValue, newText, isTrigger) {
            var g = this, p = this.options;
        ...
        _setAutocomplete: function (value)
        {
            ...
            g.inputText.blur(function(){
            	if (this._acto1)
                    clearTimeout(this._acto1);
            	this._acto1 = setTimeout(function(){
            		...
            		var key = g.inputText.val();
            		if (key){
            			if (p.onButtonClick){
            				p.onButtonClick.call(g, key);
            				//g.trigger(this, function(){
//            					p.onButtonClick(key);
//            				});
            			}else{
            				g.inputText.val("");
...
(function ($)
{
    ...
    $.ligerDefaults.Form.editors.textarea =
    {
        create: function (container, editParm, p)
        {
            ...
            var id = (p.prefixID || "") + editParm.field.name.replace("\.", "_");
            if ($("#" + id).length)
            {
                editor = $("#" + id);
            }
    ...
    $.ligerDefaults.Form.editorBulider = function (jinput) {
        ...
        var optionsStr = jinput.attr("options");
        if(optionsStr) {
        	options = $.extend(options, eval("(" + optionsStr + ")"));
        }
        ...
        if (jinput.is("select")) {
            ...
            switch (ltype) {
                ...
                case "date":
                    ...
                    break;
                case "dateRange":
                    jinput.ligerDateRangeEditor(options);
                    break;
                case "popup":
                ...
                case "autocompleteText":
                	ligerAutocompleteText(jinput,options);
                	break;
                default:
                    jinput.ligerTextBox(options);
                    break;
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            g.form = jform;//jform.is("form") ? jform : jform.parents("form:first");
            //生成ligerui表单样式
...
(function ($)
{
    ...
    $.ligerDefaults.Grid.formatters['int'] = function (value, column){
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadData: function (loadDataParm)
        {
            ...
            if	(p.dataAction == "server" && p.columns.length) {
            	var field = [];
            	var dict = [];
            	var i18n = []; 
            	var dateFormat = [];
            	...
            	$(p.columns).each(function() {
					...
					if(this.dateFormat) { 
						dateFormat.push(this.dateFormat);
					}else{
						dateFormat.push("");
					}
					
					if(this.convertKey) { 
						convertKey.push(this.convertKey);
					}else{
						convertKey.push("");
					}
					
					if(this.convertType) { 
						convertType.push(this.convertType);
					}else{
						convertType.push("");
					}
            	});
            	param.push({ name: "field", value: field});
            	param.push({ name: "dict", value: dict});
				param.push({ name: "i18n", value: i18n});
				param.push({ name: "dateFormat", value: dateFormat});
				param.push({ name: "convertKey", value: convertKey});
				param.push({ name: "convertType", value: convertType});
            }
            if (p.dataAction == "server")
            {
                if (p.usePager)
        ...
        */
        ...
        _createHeaderCell: function (column)
        {
            ...
            {
            	jcell.css("line-height", p.headerRowHeight + "px");
            ...
            // 将规格头的名称去掉
            if (!column['__leaf'] && column['__colSpan'] > 1) {
                jcell.css("height", "0");
                $(">div:first", jcell).height(0);
            }
            return jcell;
        },
        ...
        _fillGridBody: function (data, frozen)
        {
            ...
            if (g.enabledGroup()) //启用分组模式
            {
                ...
                $(groupsdata).each(function (i, item)
                {
                    ...
                    if (p.groupRender)
                        gridhtmlarr.push(p.groupRender(groups[i], item, p.groupColumnDisplay, i));
                    else
        ...
        _getHtmlFromData: function (data, frozen)
        {
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
                ...
                if (p.rowClass)gridhtmlarr.push(p.rowClass(item, rowid));
...
//            }
        ...
        _applyEditor: function (obj)
        {
            ...
            {
            	if (column.editor.type == "numberbox2" && !($(rowcell).attr("allowEdit")=="true")){
            		return;
            	}
                ...
                g.bind('sysEndEdit', function ()
                {
                    ...
                    if (newValue != currentdata)
                    {
                        ...
                        {
                        	 if (column.editor.type!="autoCompleteText"){
                             	editParm.value = newValue;
                             }
                        	 if (column.editor.type=="numberbox2"){
        ...
        // 获取某一列的统计结果，包含sum count max min
        _getTotalCell: function (column, data)
        {
            var g = this, p = this.options;
            var totalsummary = {};
            if (column.totalSummary)
            {
                ...
                if (column.totalSummary.type) {
                    var types = column.totalSummary.type.split(',');
                    if (isExist('sum'))
                    	totalsummary.Sum = sum.toFixed(2);
                    if (isExist('count'))
                    	totalsummary.Count = count;
                    if (isExist('max'))
                    	totalsummary.Max = max.toFixed(2);
                    if (isExist('min'))
                    	totalsummary.Min = min.toFixed(2);
                    if (isExist('avg'))
                    	totalsummary.Avg = avg.toFixed(2);
                }
            }
            return totalsummary;
        },
...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addTabItem: function (options)
        {
            ...
            tabitem.attr("tabid", tabid);
            var parentTabid = options.parentTabid;
            if (parentTabid){
            	tabitem.attr("parentTabid",parentTabid);
            	contentitem.attr("parentTabid",parentTabid);
            }
            contentitem.attr("tabid", tabid); 
            if (url)
            {
...
(function ($)
{
    ...
    $.ligerui.controls.TextBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            g.wrapper = g.inputText.wrap('<div class="l-text"></div>').parent();
            if (p.afterContent) {
            	var dom = $("<span>" + p.afterContent + "</span>");
            	dom.insertAfter(g.inputText);
            	var w = dom.width() + 5;
            	dom.css({position: "absolute", right: -w + "px"});
            }
            g.wrapper.append('<div class="l-text-l"></div><div class="l-text-r"></div>');
            if (!g.inputText.hasClass("l-text-field"))
                g.inputText.addClass("l-text-field");
...
(function ($)
{
    ...
    $.ligerui.controls.ToolBar.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addItem: function (item)
        {
            ...
            if (item.img)
            {
                ditem.append("<span class='icon " + item.img + "' ></span>");
