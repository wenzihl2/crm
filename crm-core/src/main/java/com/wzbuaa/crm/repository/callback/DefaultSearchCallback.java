...
import com.wzbuaa.crm.repository.support.JoinTable;
import framework.entity.search.SearchOperator;
import framework.entity.search.Searchable;
import framework.entity.search.filter.AndCondition;
import framework.entity.search.filter.Condition;
import framework.entity.search.filter.OrCondition;
import framework.entity.search.filter.SearchFilter;
...
public class DefaultSearchCallback implements SearchCallback {
    ...
    public void joinQL(StringBuilder ql, Searchable search) {
        ...
        for (SearchFilter searchFilter : search.getSearchFilters()) {
            if (searchFilter instanceof Condition) {
                ...
                if (!condition.getSearchProperty().startsWith("$$") ||
                		condition.getValue() == null) {
                    continue;
                }
                JoinTable joinTable = new JoinTable(condition.getSearchProperty());
                if(ql.indexOf("1=1") != -1) {
                	ql = ql.delete(ql.indexOf(" 1=1"), ql.length() - 1);
                }
                ql = ql.insert(ql.indexOf("where"), "left join x." + joinTable.getTable() + " j ");
                if(!ql.toString().trim().endsWith("where")) {
                	ql.append(" and ");
                }
            }
        }
    ...
    public static void main(String[] args) {
    	String FIND_QUERY_STRING = "from %s x where 1=1 ";
    	Searchable search = Searchable.newSearchable().addSearchFilter("$$products.no", SearchOperator.eq, "1");
		new DefaultSearchCallback().joinQL(new StringBuilder(FIND_QUERY_STRING), search);
	}
    ...
    private int genCondition(StringBuilder ql, int paramIndex, SearchFilter searchFilter) {
        ...
        if (searchFilter instanceof Condition) {
            ...
            if (entityProperty.startsWith("$$")){
            	JoinTable joinTable = new JoinTable(condition.getSearchProperty());
            	entityProperty = "j." + joinTable.getField();
            } else {
            	entityProperty = "x." + entityProperty;
            }
    ...
    public void prepareOrder(StringBuilder ql, Searchable search) {
        if (search.hashSort()) {
            ...
            for (Sort.Order order : search.getSort()) {
                ql.append(String.format("%s%s %s, ", getAliasWithDot(), "x." + order.getProperty(), order.getDirection().name().toLowerCase()));
            }
