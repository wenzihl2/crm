...
import com.wzbuaa.crm.service.sso.UserStatusHistoryService;
...
public class LoginFormController {
    ...
    @Autowired private UserStatusHistoryService userStatusHistoryService;

    @RequestMapping(value = {"/admin/{login:login;?.*}"}) //spring3.2.2 bug see  http://jinnianshilongnian.iteye.com/blog/1831408
    public String loginForm(HttpServletRequest request, ModelMap model) {

        //表示退出
        if (!StringUtils.isEmpty(request.getParameter("logout"))) {
            model.addAttribute(Constants.MESSAGEKEY, Message.success("user.logout.success", null));
        }

        //表示用户删除了 @see org.apache.shiro.web.filter.user.SysUserFilter
        if (!StringUtils.isEmpty(request.getParameter("notfound"))) {
            model.addAttribute(Constants.ERROR, Message.error("user.notfound", null));
        }

        //表示用户被管理员强制退出
        if (!StringUtils.isEmpty(request.getParameter("forcelogout"))) {
            model.addAttribute(Constants.ERROR, Message.error("user.forcelogout", null));
        }

        //表示用户输入的验证码错误
        if (!StringUtils.isEmpty(request.getParameter("jcaptchaError"))) {
            model.addAttribute(Constants.ERROR, Message.error("jcaptcha.validate.error", null));
        }
        ...
        if (!StringUtils.isEmpty(request.getParameter("blocked"))) {
            ...
            model.addAttribute(Constants.ERROR, Message.error("user.blocked", new Object[]{reason}));
        }

        if (!StringUtils.isEmpty(request.getParameter("unknown"))) {
            model.addAttribute(Constants.ERROR, Message.error("user.unknown.error", null));
        }
        ...
        if (model.containsAttribute(Constants.ERROR)) {
            model.remove(Constants.MESSAGEKEY);
        }
