package com.wzbuaa.crm.component.service;
 ...
 * <p>Version: 1.0
 */
public abstract class BaseMovableService<M extends AbstractEntity & Movable, ID extends Serializable> extends BaseService<M, ID> {
    ...
    public void down(ID fromId, ID toId) {
        ...
        Integer fromWeight = from.getPriority();
        Integer toWeight = to.getPriority();
        ...
        if (from.equals(nextTo)) {
            from.setPriority(toWeight);
            to.setPriority(fromWeight);
            return;
        }
        ...
        if (count > 0 && count < 20) {
            ...
            if (fromWeight < toWeight) {
                Integer swapInteger = moves.get(count - 2).getPriority();
                for (int i = count - 2; i >= 1; i--) {
                    //最后一个的weight = toWeight;
                    moves.get(i).setPriority(moves.get(i - 1).getPriority());
                }
                moves.get(0).setPriority(swapInteger);
            } else {
                for (int i = 0; i <= count - 2; i++) {
                    moves.get(i).setPriority(moves.get(i + 1).getPriority());
                }
                moves.get(count - 1).setPriority(minWeight);
            }
            return;
        }
        ...
        if (preTo == null) {
            ...
            newWeight = toWeight - (toWeight - preTo.getPriority()) / 2;
    ...
    public void up(ID fromId, ID toId) {
        ...
        if (count > 0 && count < 20) {
            ...
            if (fromWeight > toWeight) {
                ...
                moves.get(count - 1).setPriority(maxWeight);
            }
            return;
        }
        ...
        if (nextTo == null) {
            ...
            newWeight = toWeight + (nextTo.getPriority() - toWeight) / 2;
        }
    ...
    public void doReweight(Page<M> page) {
        ...
        for (int i = 0; i < moves.size(); i++) {
            M move = moves.get(i);
            move.setPriority((totalElements - firstElement - i) * stepLength);
            update(move);
        }
    ...
    private Integer findNextWeight() {
        Searchable searchable = Searchable.newSearchable().setPage(0, 1).addSort(Sort.Direction.DESC, "priority");
        ...
        return page.getContent().get(0).getPriority() + stepLength;
    }
    ...
    List<M> findByBetweenAndAsc(Integer minWeight, Integer maxWeight) {
        ...
        return findAllWithSort(Searchable.newSearchable(searchParams).addSort(Sort.Direction.ASC, "priority"));
    }
