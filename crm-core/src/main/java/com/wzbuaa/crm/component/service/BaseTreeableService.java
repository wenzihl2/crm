...
import framework.entity.search.filter.SearchFilterHelper;
...
public abstract class BaseTreeableService<M extends BaseEntity<ID> & Treeable<ID>, ID extends Serializable>
	...
	protected BaseTreeableService() {
        ...
        FIND_SELF_AND_NEXT_SIBLINGS_QL =
                String.format("from %s where parentIds = ?1 and priority>=?2 order by priority asc", entityName);

        FIND_NEXT_WEIGHT_QL =
                String.format("select case when max(priority) is null then 1 else (max(priority) + 1) end from %s where parentId = ?1", entityName);
    ...
    public M save(M m) {
        if (m.getPriority() == null) {
            m.setPriority(nextPriority(m.getParentId()));
        }
        return super.save(m);
    }
    ...
    public void appendChild(M parent, M child) {
        ...
        child.setPriority(nextPriority(parent.getId()));
        save(child);
    }

    public int nextPriority(ID id) {
    ...
    public void move(M source, M target, String moveType) {
        ...
        if (isSibling && isNextOrPrevMoveType && Math.abs(source.getPriority() - target.getPriority()) == 1) {
            ...
            int sourceWeight = source.getPriority();
            source.setPriority(target.getPriority());
            target.setPriority(sourceWeight);
            return;
        }

        //移动到目标节点之后
        if ("next".equals(moveType)) {
            List<M> siblings = findSelfAndNextSiblings(target.getParentIds(), target.getPriority());
            siblings.remove(0);//把自己移除

            if (siblings.size() == 0) { //如果没有兄弟了 则直接把源的设置为目标即可
                int nextWeight = nextPriority(target.getParentId());
        ...
        if ("prev".equals(moveType)) {
            ...
            if (siblings.contains(source)) {
                ...
                updateSelftAndChild(source, target.getParentId(), target.getParentIds(), source.getPriority());
            }

            return;
        }
        //否则作为最后孩子节点
        int nextWeight = nextPriority(target.getId());
