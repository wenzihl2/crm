...
<title>会员中心-安全验证</title>
[#include "/shop/common/include.ftl" /]
<script type="text/javascript">
	//提交订单
	function dosubmit() {
		if (checkMobile() && checkRecoverKey()) {
			$.ajax({
				url : "${base}/member/safe/verifyMobile",
				type : 'post',
				data : {
					"mobile" 		: $("#member_mobile").val(),
					"smsRecoverKey" : $("#member_recoverKey").val(),
				},
	...
	function sendMessage() {
		...
		$.ajax({
			url : base + "/member/safe/${smsMsgType}/sendSms",
			type : 'post',
			dataType : "json",
			success : function(data) {
				if (data.type == "SUCCESS") {
				...
				<div class="mydd_c">
					<div id="step2" class="step">
						<ul>
							...
							<li class="fore2">2.修改登录密码</li>
							<li class="fore2">3.完成</li>
						</ul>
					</div>
					<form id="inputForm" action="${smsMsgType}/validSms" method="post">
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0"  class="liuyan">
							<tbody>
								<tr>
									...
									<td>
										<input type="text" size="30" class="inputBg" value="${member.mobile[0..2]+'****'+member.mobile[7..10]}" disabled="true">
										<input type="hidden" name="mobile" id="member_mobile" value="${member.mobile}"/>
								...
								<tr>
									...
									<td height="40" style="padding-left:7px">
