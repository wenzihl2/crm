...
$.extend($.ligerDefaults.Grid, {
    ...
    pageParmName: 'page.pageNumber',//页索引参数名，(提交给服务器)
    pagesizeParmName: 'page.size', //页记录数参数名，(提交给服务器)
...
$.ligerDefaults.Grid.formatters['numberbox2'] =
...
//扩展一个 数字输入 的编辑器
$.ligerDefaults.Grid.editors['numberbox2'] = 
$.ligerDefaults.Grid.editors['numberbox'] = {
    ...
    setValue: function (input, value, editParm) {
        ...
        if(jQuery.isNumeric(value)) {
        	...
        	input.val(num.toFixed(precision));
        }
    },
    resize: function (input, width, height, editParm) { 
        input.width(width).height(height);
...
$.ligerDefaults.Grid.editors['select'] =
{
    create: function (container, editParm)
    {
        ...
        $(data).each(function ()
        {
            input.append('<option value="' + (this.id || this.value) + '">' + (this.text || this.name || this.value) + '</option>');
        });
        return input;
    },
    getValue: function (input, editParm)
...
$.ligerDefaults.Grid.editors['int2'] =
...
$.ligerDefaults.Grid.editors['file'] = {
    create: function (container, editParm)
    {
        var input = $("<input type='file' class='l-text-editing'/>");
        if (typeof (editParm.column.validate) == "string")
        {
            input.attr("validate", editParm.column.validate);
        }
...
$.ligerDefaults.Grid.editors['autoCompleteText'] = {
    create: function (container, editParm) {
        var column = editParm.column;
        var input = $("<input type='text'/>");
        container.append(input);
        ...
        var g = input.ligerComboBox(options);
        var pop = $("div.l-grid-editor .l-trigger-icon");
        pop.removeClass('l-trigger-icon').addClass('l-trigger-icon-autocomplete');
        pop.parent().parent().unbind('mouseenter').unbind("mouseleave");
        pop.parent().unbind();
        g.link.unbind();
        g.link.bind("click", function(){
        	column.editor.popfunc(column);
        });
        return input;
    },
    getValue: function (input, editParm) {
