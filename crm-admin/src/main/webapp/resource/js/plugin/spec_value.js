var SpecValue = {
	init:function(){
		var self =this;
		$(".specList input[name=specids]").attr("checked", false);
		//添加规格值
		$(".specList input[name=specids]").click(function(){
			self.specClick($(this));
		});
		//生成货品
	...
	addSpec: function(spec_id, spec_name){
		var self =this;
		$.ajax({
			url: "spec_values.jhtml",
			data:{"id": spec_id},
			dataType:"html",
			sync: false,
			success:function(spechtml){
				if($(".spec_tab .tab-bar ul.tab li").size() > 0) {
					return;
				}
	...
	addOne: function(spec_id, span){
		var value_id = span.attr("specvid")
		//已经有的规格不添加
	 	var isExist = false;
	...
	//删除一个规格
	deleteOne:function(delBtn, spec_box){
		delBtn.parents("tr").remove();
		spec_box.removeClass("selected");
	},
	//生成货品
	createPros:function(){
		var specs=[];
		...
		//生成货品列表
		parent.Product.createPros(specs, names);
