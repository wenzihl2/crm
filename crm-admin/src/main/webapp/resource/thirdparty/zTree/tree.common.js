$.zTree = {
    ...
    autocompleteTemplate :
        '<table style="margin:10px 0 0 10px;"><tr>' +
            '<th>名称:</th>' +
            '<td><input type="text" id="searchName{id}" ltype="autocomplete" placeholder="模糊匹配 回车键查询"/></td>' +
        '</tr></table>',
    ...
    initMovableTree : function(config) {
        ...
        config.renameUrl = config.renameUrl || (config.urlPrefix + "/ajax/{id}/rename.jhtml?newName={newName}");
        config.removeUrl = config.removeUrl || (config.urlPrefix + "/ajax/{id}/delete.jhtml");
        config.addUrl = config.addUrl || (config.urlPrefix + "/ajax/{id}/appendChild.jhtml");
        config.moveUrl = config.moveUrl || (config.urlPrefix + "/ajax/{sourceId}/{targetId}/{moveType}/move.jhtml");
        config.asyncLoadAll = config.asyncLoadAll || false;
        config.loadUrl = config.loadUrl || (config.urlPrefix + "/ajax/load.jhtml" +
                "?async=" + config.async +
        ...
        function onAdd(e, treeId, treeNode) {
            ...
            $.ajax({"url": url, success:function(newNode){
...
//                });
//            };
            config.autocomplete.url = config.autocomplete.url
                || config.urlPrefix + "/ajax/autocomplete.jhtml?1=1" +
                  (config.excludeId ? "&excludeId=" + config.excludeId : "") +
                  (config.onlyDisplayShow ? "&search.show_eq=true" : "");
			$("#searchName" + id).ligerComboBox({
        		url: config.autocomplete.url,
        		keySupport: true,
        		autocomplete: true
        	});
        	
        	$("#searchName" + id).on("keydown", function( event ) {
                //回车查询
                if(event.keyCode === 13) {
    ...
    initSelectTree : function(config) {
        config.asyncLoadAll = config.asyncLoadAll || false;
        config.loadUrl =
            config.loadUrl || (config.urlPrefix + "/ajax/load.jhtml" +
                "?async=" + config.async +
                "&asyncLoadAll=" + config.asyncLoadAll +
                (config.excludeId ? "&excludeId=" + config.excludeId : "") +
                (config.onlyDisplayShow ? "&search.show_eq=true" : "") +
    ...
    initMaintainBtn : function(maintainUrlPrefix, tableId, async) {
        var updateUrl = maintainUrlPrefix + "/{id}/edit.jhtml",
            deleteUrl = maintainUrlPrefix + "/batch/delete.jhtml",
            moveTreeUrl = maintainUrlPrefix + "/{id}/move.jhtml?async=" + async;
        ...
        $("#deleteTree").off("click").on("click", function () {
            ...
            if(checkbox.filter("[root='true']").length) {
            	LG.tip("您删除的数据中包含根节点，根节点不能删除！");
                return;
            }
            
            $.ligerDialog.confirm("确认删除吗？", "提示", function(){
